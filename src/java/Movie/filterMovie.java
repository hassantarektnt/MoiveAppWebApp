       ...
       /* HashMap <String , Movie> movies = (HashMap <String , Movie> )request.getServletContext().getAttribute("movies");
        ...
        List <Movie> movies =   (List <Movie> )currentSession.getAttribute("filter");
         for (int i = 0 ; i < movies.size() ; i++ ) {
            ...
            if (selector.equals("Year") && movies.get(i).getYear() == Integer.valueOf(keyword) ) {
                result += movies.get(i).getPosterUrl() ;
                filter.add(movies.get(i)) ;
                        
            }
            
            else if (movies.get(i).getMainActor().toLowerCase().equals(keyword.toLowerCase()) && selector.equals("Actor")) {
                result += movies.get(i).getPosterUrl() ;
                filter.add(movies.get(i)) ;
            }
            
             else if (movies.get(i).getMainActress().toLowerCase().equals(keyword.toLowerCase())) {
                result += movies.get(i).getPosterUrl() ;
                filter.add(movies.get(i)) ;
            }
