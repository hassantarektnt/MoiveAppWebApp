    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String moviename = request.getParameter("name");
         ...
         for ( HashMap.Entry<String, Movie> movie : movies.entrySet()) {
             if (movie.getKey().toLowerCase().equals(moviename.toLowerCase())) {
                 movieposter = movie.getValue().getPosterUrl() ;
                //response.getWriter().write(movie.getValue().posterURL);
                 break ;
             }
             
         }
         out.print("sdvdvdva"+movieposter);
         response.getWriter().write(movieposter);
